  ...
  return (
    <section className="premium-card p-4">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-teal-700 dark:text-teal-300">Lokal gespeichert</p>
          <h2 className="mt-1 text-lg font-semibold tracking-normal text-slate-950 dark:text-white">Verlauf</h2>
        </div>
        {history.length ? (
          <button
            type="button"
          ...
          {history.map((item) => (
            ...
            >
              ...
              <span className="mt-1 block truncate text-xs text-slate-500 dark:text-slate-400">
                {new Date(item.createdAt).toLocaleString("de-DE")} · {item.value}
              </span>
            </button>
          ))}
        </div>

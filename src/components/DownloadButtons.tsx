...
import type { HistoryItem, QRDesignOptions, QRExportFormat, QRQualityResult, StoredSettings, ValidationResult } from "../types";
...
const formats: { format: QRExportFormat; label: string; shortLabel: string; icon: typeof FileImage }[] = [
  { format: "png", label: "PNG herunterladen", shortLabel: "PNG", icon: ImageDown },
  { format: "svg", label: "SVG herunterladen", shortLabel: "SVG", icon: FileText },
  { format: "jpeg", label: "JPEG herunterladen", shortLabel: "JPEG", icon: FileImage },
  { format: "webp", label: "WebP herunterladen", shortLabel: "WebP", icon: FileImage },
  { format: "pdf", label: "PDF herunterladen", shortLabel: "PDF", icon: FileArchive },
];
...
export function DownloadButtons({
  ...
  const runDownload = async (format: QRExportFormat) => {
    ...
    if (!validation.isValid) {
      setStatus("Bitte behebe zuerst die Eingabefehler.");
      return;
    }
    try {
      await downloadQRCode(qrCodeRef.current, format, fileName, design);
      setStatus(quality.level === "Kritisch" ? "Download erstellt. Bitte teste den QR-Code besonders sorgfältig." : "Download erstellt.");
      onDownloaded();
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Download konnte nicht erstellt werden.");
    }
  };
  ...
  const exportJson = () => {
    ...
    setToolsStatus("JSON exportiert.");
  };
  ...
  const importJson = (file: File) => {
    ...
    reader.onload = () => {
      try {
        ...
        setToolsStatus("Einstellungen importiert.");
      } catch {
        setToolsStatus("Die JSON-Datei passt nicht zu dieser App.");
      }
    };
  ...
  const qualityTone =
    quality.level === "Sehr gut" || quality.level === "Gut"
      ...
      <div className="mx-auto flex w-full max-w-[1480px] flex-col gap-2 lg:flex-row lg:items-stretch">
          ...
          <div className={`rounded-md border px-3 py-1.5 ${qualityTone}`}>
            <div className="flex min-h-7 items-center justify-between gap-2">
              <span className="flex min-w-0 items-center gap-2 text-[13px] font-semibold">
                {quality.level === "Sehr gut" || quality.level === "Gut" ? <Check aria-hidden="true" className="h-4 w-4" /> : <AlertTriangle aria-hidden="true" className="h-4 w-4" />}
                <span className="truncate">Qualität: {quality.level}</span>
              </span>
              <span className="flex items-center gap-2">
                ...
                <span className="group relative inline-flex">
                  ...
                  <span className="pointer-events-none absolute bottom-[calc(100%+0.6rem)] right-0 z-[70] w-[320px] rounded-lg border border-slate-200 bg-white p-3 text-left text-sm leading-5 text-slate-700 opacity-0 shadow-2xl shadow-slate-950/15 transition group-hover:opacity-100 group-focus-within:opacity-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">
                    <span className="mb-1 block text-xs font-bold uppercase tracking-wide text-teal-700 dark:text-teal-300">Hinweise</span>
                    {tooltipItems.length ? (
                      ...
                      <span>Keine Hinweise. Der QR-Code ist bereit.</span>
                    )}
                  </span>
                </span>
              </span>
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-center rounded-lg border border-teal-200/90 bg-gradient-to-br from-teal-50/95 via-white/90 to-sky-50/90 p-2 shadow-panel dark:border-teal-900/60 dark:from-teal-950/32 dark:via-slate-900/88 dark:to-sky-950/28">
          <div className="grid gap-2 sm:grid-cols-[minmax(160px,240px)_minmax(0,1fr)] xl:grid-cols-[minmax(160px,240px)_minmax(0,1fr)_auto] xl:items-center">
            <label className="grid gap-1 xl:gap-0">
              <span className="text-[11px] font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">Dateiname</span>
              <input
            ...
            <div className="grid gap-2 sm:grid-cols-2 xl:flex xl:min-w-0 xl:items-center">
              {visibleFormats.map(({ format, label, shortLabel, icon: Icon }) => (
                <button
                  key={format}
                  type="button"
                  title={label}
        ...
        <div className="flex min-w-[260px] flex-col justify-center rounded-lg border border-indigo-200/90 bg-gradient-to-br from-indigo-50/90 via-white/90 to-amber-50/75 p-2 shadow-panel dark:border-indigo-900/60 dark:from-indigo-950/35 dark:via-slate-900/88 dark:to-amber-950/22 lg:w-[320px]">
          <div className="mb-1.5 flex items-center justify-between gap-3">
            <p className="text-[11px] font-bold uppercase tracking-wide text-teal-700 dark:text-teal-300 lg:hidden">Verlauf & Einstellungen</p>
            {history.length ? (
              <button
                type="button"
          ...
          <div className="grid gap-2 lg:hidden">
            {history.length ? (
              ...
              <p className="rounded-md border border-slate-100 bg-white/72 px-3 py-2 text-sm leading-5 text-slate-600 shadow-sm dark:border-slate-800 dark:bg-slate-950/45 dark:text-slate-300">
                Nach dem ersten Download erscheint hier dein lokaler Verlauf.
              </p>
            )}
          </div>
            ...
            <button
              ...
              onClick={() => {
                void copyToClipboard(settingsJson).then((ok) => {
                  ...
                  setToolsStatus(ok ? "Einstellungen kopiert." : "Zwischenablage ist nicht verfügbar.");
